...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SOURCE PARAMETERS
lam0 = [400:5:800] * nanometers; %free space wavelength
Energy=h*c./lam0;
...
er2 = nSiw(lam0(ii)/nanometers)^2; %permittivity in transmission region

n_layers_unitcell = 1;
n_bilayers = 1;
urd =[1.0 1.0 1.0]; %permeability of device
erd =[(n_alphaSiw(lam0(ii)/nanometers)/2+er1/2)^2 n_alphaSiw(lam0(ii)/nanometers)^2 nSiO2w(lam0(ii)/nanometers)^2];
ds = 10.15 * nanometers;
d1 = 52.53 * nanometers;
d2 = 300 * nanometers;
%df = 120 * nanometers;

Lx = 320 * nanometers; %period along x
Ly = 320 * nanometers; %period along y

% RCWA PARAMETERS
Nx = 320; %number of point along x in real-space grid
...
UR(1:Nx,1:Ny,1) = urd(2);% zeros(Nx,Ny,NL);
UR(1:Nx,1:Ny,2) = urd(2);
ER(1:Nx,1:Ny,1) = erd(2);% zeros(Nx,Ny,NL);
ER(1:Nx,1:Ny,2) = erd(2);
UR(1:Nx,1:Ny,3) = urd(3);
ER(1:Nx,1:Ny,3) = erd(3);

 w = (320-240)/320; %length of one side of triangle
