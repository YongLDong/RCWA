...
erd =[n_alphaSiw(lam0(ii)/nanometers)^2 nCuw(lam0(ii)/nanometers)^2 nSiO2w(lam0(ii)/nanometers)^2];
d1 = 100 * nanometers;
d2 = 40 * nanometers;
d3 = 40 * nanometers;
%df = 120 * nanometers;
...
% RCWA PARAMETERS
Nx = 100; %number of point along x in real-space grid
...
UR(1:Nx,1:Ny,1) = ur2;% zeros(Nx,Ny,NL);
UR(1:Nx,1:Ny,2) = ur2;
ER(1:Nx,1:Ny,1) = er2;% zeros(Nx,Ny,NL);
ER(1:Nx,1:Ny,2) = er2;
UR(1:Nx,1:Ny,3) = ur2;
ER(1:Nx,1:Ny,3) = er2;
...
ER(nx1+1:nx2,1:Ny,3) = erd(3);
% 
ny = round(f*w*Ny);
ny1 = floor((Ny - ny)/2);
ny2 = ny1 + ny;

ER(1:Nx,ny1+1:ny2,1) = erd(3);
